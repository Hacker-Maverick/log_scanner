...
# --- Log_Scanner: GitHub Repository Initializer ---
...
if [ -f "log_scanner.py" ]; then
    mv log_scanner.py core/
    echo -e "\e[32m[+] Moved engine to core/\e[0m"
fi

if [ -f "log_scanner.sh" ]; then
    mv log_scanner.sh scripts/
    chmod +x scripts/log_scanner.sh
...
Log_Scanner is a security tool designed for real-time network auditing, credential leak detection, and DNS exfiltration monitoring. It features a professional Terminal User Interface (TUI) and uses mathematical entropy analysis to detect stealthy data exfiltration.
...
# git init
# git add .
# git commit -m "Initial commit: Log_Scanner v3.5 Architecture"
